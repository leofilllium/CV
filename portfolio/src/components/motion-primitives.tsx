...
export function Magnetic({
  ...
  const x = useMotionValue(0),
    y = useMotionValue(0);
  const springX = useSpring(x, { stiffness: 220, damping: 19 }),
    springY = useSpring(y, { stiffness: 220, damping: 19 });
  return (
    <motion.span
...
export function Manifesto({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="manifesto" ref={ref} aria-hidden="true">
      <motion.div style={{ x: calm ? 0 : x }}>{children}</motion.div>
    </div>
  );
}
